...
window.addEventListener('DOMContentLoaded', () => {
  ...
  async function refreshList() {
    ...
    try {
      ...
      files.forEach(f => {
        ...
        try { li.title = 'Doble clic: cargar en Deck A (Shift: Deck B)'; } catch {}
        ...
        if (window.__lastNewFile && window.__lastNewFile === f) {
          try {
            ...
            setTimeout(() => { try { li.classList.remove('new-pulse'); } catch {} }, 4000);
          } catch {}
        }
        // Botones por elemento para cargar en Decks
        const btnA = li.querySelector('.btn-load-a');
        if (btnA) btnA.addEventListener('click', () => { if (typeof deckA !== 'undefined' && deckA) deckA.setSrc(`/output/${encodeURIComponent(f)}`); });
        const btnB = li.querySelector('.btn-load-b');
        if (btnB) btnB.addEventListener('click', () => { if (typeof deckB !== 'undefined' && deckB) deckB.setSrc(`/output/${encodeURIComponent(f)}`); });
      });
  ...
  try { window.refreshOutputList = refreshList; } catch {}
  const outFilterEl = document.getElementById('output-filter');
  if (outFilterEl && !outFilterEl.dataset.bound) { outFilterEl.addEventListener('input', refreshList); outFilterEl.dataset.bound = '1'; }
  ...
  async function createAlbum() {
    ...
    try {
      ...
      if (data.ok && data.albumId) {
        ...
        if (mintBtn) { mintBtn.disabled = false; mintBtn.dataset.albumId = data.albumId; }
        if (pubBtn) { pubBtn.disabled = false; pubBtn.dataset.albumId = data.albumId; }
      }
    } catch (e) {
  ...
  function setHStatus(msg) { if (hederaResult) hederaResult.textContent = msg; }
  function getHInputs() {
    return {
  ...
  function validateIdLike(v) { return /^\d+\.\d+\.\d+$/.test((v||'').trim()); }
  function setLoading(btn, loadingText) { if (!btn) return () => {}; const prev = btn.textContent; btn.disabled = true; btn.textContent = loadingText || 'Trabajando...'; return () => { btn.disabled = false; btn.textContent = prev; }; }
  ...
  if (btnNftList) btnNftList.addEventListener('click', async () => {
    ...
    try {
      ...
      if (listPre) { listPre.textContent = ''; listPre.dataset.next = ''; }
      setHStatus('Error: ' + e.message);
    }
  });
  ...
  if (btnCopy) btnCopy.addEventListener('click', async () => {
    ...
    if (!val) { setHStatus('Nada para copiar.'); return; }
    try {
      await navigator.clipboard.writeText(val);
      setHStatus('Base64 copiada al portapapeles.');
    } catch (e) {
      setHStatus('No se pudo copiar: ' + e.message);
    }
  });
  ...
  if (btnMore) btnMore.addEventListener('click', async () => {
    ...
    if (!next) { setHStatus('No hay más páginas.'); return; }
  ...
  function setSplit(msg) { if (splitRes) splitRes.textContent = msg; }
  ...
  if (btnAddSplit) btnAddSplit.addEventListener('click', () => { renderSplitRow('', 0); updateBpsSum(); });
  if (splitsContainer && !splitsContainer.hasChildNodes()) { renderSplitRow('', 5000); renderSplitRow('', 5000); updateBpsSum(); }

  let lastTokenId = '';
  let lastSerial = 0;

  if (btnSplitCreate) btnSplitCreate.addEventListener('click', async () => {
  ...
  if (btnSplitMint) btnSplitMint.addEventListener('click', async () => {
    ...
    if (meta) {
      try { metadataJson = JSON.parse(meta); } catch { metadataJson = { description: meta+'' }; }
    }
  ...
  if (btnSplitAssociate) btnSplitAssociate.addEventListener('click', async () => {
    ...
    try {
      ...
      setSplit('Error associate: ' + e.message);
    } finally { stop(); }
  });
  ...
  if (btnSplitTransfer) btnSplitTransfer.addEventListener('click', async () => {
    ...
    try {
      ...
      setSplit('Error transfer: ' + e.message);
    } finally { stop(); }
  });

  // Esqueleto Wallet/HashConnect
  const walletStatusEl = document.getElementById('wallet-status');
  function setWStatus(msg) { if (walletStatusEl) walletStatusEl.textContent = msg; }

  const btnWalletConnect = document.getElementById('btn-wallet-connect');
  if (btnWalletConnect) btnWalletConnect.addEventListener('click', async () => {
    ...
    try {
      ...
      if (!hcInstance) {
        try {
          ...
          console.warn('Error creando HashConnect:', e);
        }
      }
      ...
      try {
        ...
        console.warn('Error en init/connect HashConnect:', e);
      }
  ...
  if (btnWalletSign) btnWalletSign.addEventListener('click', async () => {
    const area = document.getElementById('assoc-tx-base64');
    ...
    try {
      ...
      let txBase64 = base64;
      if (!txBase64) {
        if (!accountId || !tokenId) { setWStatus('Completa accountId y tokenId o pega base64.'); return; }
        const data = await callApi('/api/hedera/associate/prepare', { method: 'POST', body: JSON.stringify({ accountId, tokenId }) });
        txBase64 = data.bytes || '';
      ...
      function b64ToUint8(b64) {
        try {
          ...
          return bytes;
        } catch (e) { throw new Error('Base64 inválido: ' + e.message); }
      }
      ...
      try {
        ...
        console.warn('No se pudo cargar @hashgraph/sdk desde CDN:', e);
      ...
      try {
        ...
        setWStatus('No se pudo reconstruir la transacción: ' + e.message);
        return;
      }
      ...
      try {
        ...
        setWStatus('Wallet rechazó o falló la firma: ' + (e?.message || e));
        return;
      }
        ...
        syncSource.onmessage = (evt) => {
          try {
            ...
            if (data.type === 'hello') {
              ...
              try { window.__lastNewFile = data.file || null; } catch {}
            ...
            console.warn('Error procesando SSE:', e);
          }
        };
  ...
  if (btnAnalytics) btnAnalytics.addEventListener('click', async () => {
    try {
  ...
  function createDeck(letter) {
    ...
    function drawVis() {
      ...
      if (!ctx2d) { rafId = requestAnimationFrame(drawVis); return; }
      ...
      try { (window.__setDeckEnergy && window.__setDeckEnergy(letter, bars ? (sum / bars) : 0)); } catch {}
    ...
    if (rateEl) rateEl.addEventListener('input', () => { mediaEl.playbackRate = Number(rateEl.value || 1); });
    if (gainEl) gainEl.addEventListener('input', () => { gain.gain.value = Number(gainEl.value || 1); });
    if (filterEl) filterEl.addEventListener('input', () => { filter.frequency.value = Number(filterEl.value || 18000); });
    if (echoTimeEl) echoTimeEl.addEventListener('input', () => { delay.delayTime.value = Number(echoTimeEl.value || 0.25); });
    if (echoFbEl) echoFbEl.addEventListener('input', () => { fb.gain.value = Number(echoFbEl.value || 0.2); });
    ...
    if (btnStop) btnStop.addEventListener('click', () => { mediaEl.pause(); mediaEl.currentTime = 0; });
  ...
  if (btnClearA) btnClearA.addEventListener('click', () => { try { clearDeck(deckA); } catch {} });
  const btnClearB = document.getElementById('deckB-clear');
  if (btnClearB) btnClearB.addEventListener('click', () => { try { clearDeck(deckB); } catch {} });
  ...
  function bindDeckDrop(deck) {
    ...
    const onEnterOver = (e) => {
      try { e.preventDefault(); e.dataTransfer.dropEffect = 'copy'; } catch {}
      try { container.classList.add('deck-over'); } catch {}
    };
    const onLeave = (e) => {
      try { container.classList.remove('deck-over'); } catch {}
    };
    ...
    container.addEventListener('drop', async (e) => {
      try {
        ...
        if (f) { deck.setSrc(`/output/${encodeURIComponent(f)}`); return; }
  ...
  function clearDeck(deck) {
    ...
    try { mediaEl.removeAttribute('src'); mediaEl.src = ''; mediaEl.load(); } catch {}
    ...
    if (rateEl) { rateEl.value = '1'; mediaEl.playbackRate = 1; }
    if (gainEl && deck.gain) { gainEl.value = '1'; deck.gain.gain.value = 1; }
    if (filterEl && deck.filter) { filterEl.value = '18000'; deck.filter.frequency.value = 18000; }
    if (echoTimeEl && deck.delay) { echoTimeEl.value = '0.25'; deck.delay.delayTime.value = 0.25; }
    if (echoFbEl && deck.fb) { echoFbEl.value = '0.2'; deck.fb.gain.value = 0.2; }
  ...
  if (btnLoadALocal) btnLoadALocal.addEventListener('click', () => {
    const f = getSelectedAt(0);
  ...
  if (btnLoadBLocal) btnLoadBLocal.addEventListener('click', () => {
    const f = getSelectedAt(1) || getSelectedAt(0);
  ...
  if (btnMixerSearch) btnMixerSearch.addEventListener('click', async () => {
    ...
    try {
      ...
      if (!sources.length) { mixerResults.innerHTML = '<li>Sin resultados.</li>'; return; }
      for (const s of sources) {
        const li = document.createElement('li');
        li.innerHTML = `
  ...
  if (btnGithubSearch) btnGithubSearch.addEventListener('click', async () => {
    ...
    try {
      ...
      if (!data.ok) { githubResults.innerHTML = `<li>Error: ${data.error || 'desconocido'}</li>`; return; }
      const items = data.items || [];
      if (!items.length) { githubResults.innerHTML = '<li>Sin resultados.</li>'; return; }
  ...
  if (btnItunesSearch) btnItunesSearch.addEventListener('click', async () => {
    ...
    try {
      ...
      if (!data.ok) { itunesResults.innerHTML = `<li>Error: ${data.error || 'desconocido'}</li>`; return; }
      const items = data.results || [];
      if (!items.length) { itunesResults.innerHTML = '<li>Sin resultados.</li>'; return; }
...
(function reorderPanelsOnceReady() {
  ...
  let midiMap = {};
  try { midiMap = JSON.parse(localStorage.getItem('midiMap') || '{}') || {}; } catch { midiMap = {}; }
  function setMidiStatus(t) { if (midiStatusEl) midiStatusEl.textContent = t || ''; }
  ...
  function handleMidiMessage(e) {
    ...
    if (midiLearnX) {
      if (type === 0xB0) {
        ...
        try { localStorage.setItem('midiMap', JSON.stringify(midiMap)); } catch {}
    ...
    if (midiMap.xfader && midiMap.xfader.type === 'cc' && type === 0xB0) {
      if (midiMap.xfader.channel === ch && midiMap.xfader.cc === d1) {
        ...
        if (xfaderEl) { xfaderEl.value = String(v); updateXfader(); }
      }
    }
  }
    ...
    try {
      ...
      if (midiInputsSel && !midiInputsSel.dataset.bound) {
        midiInputsSel.addEventListener('change', () => {
          ...
          if (!id) return;
          for (const input of midiAccess.inputs.values()) {
            if (input.id === id) { midiInput = input; break; }
          }
          if (midiInput) {
      ...
      setMidiStatus('MIDI no disponible: ' + e.message);
    }
  }

  if (btnMidiConn) btnMidiConn.addEventListener('click', connectMIDI);
  if (btnMidiLearnX) btnMidiLearnX.addEventListener('click', () => {
  ...
  function renderPulseResult(data) {
    try {
      ...
      let href = (data && (data.jsonUrl || data.filePath || data.savedPath)) || '';
      ...
      if (pulsesPre) pulsesPre.textContent = 'Error al renderizar: ' + e.message;
    }
  }
  ...
  if (btnPulsesLatest && !btnPulsesLatest.dataset.bound) {
    btnPulsesLatest.addEventListener('click', async () => {
      ...
      try {
        ...
        if (pulsesPre) pulsesPre.textContent = 'Error: ' + e.message;
      } finally {
  ...
  if (btnPulsesGenerate && !btnPulsesGenerate.dataset.bound) {
    btnPulsesGenerate.addEventListener('click', async () => {
      ...
      try {
        ...
        renderPulseResult(data);
        try { alert('Pulso generado.'); } catch {}
      } catch (e) {
        if (pulsesPre) pulsesPre.textContent = 'Error: ' + e.message;
      } finally {
  ...
  function showBiResult(kind, data) {
    if (biPre) {
      try { biPre.textContent = JSON.stringify(data, null, 2); } catch { biPre.textContent = String(data); }
    }
  ...
  if (btnBiTrends && !btnBiTrends.dataset.bound) {
    btnBiTrends.addEventListener('click', async () => {
      ...
      try {
        ...
        try { await fetch('/api/analytics/event', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ type:'bi_ui_trends', data:{ days }})}); } catch {}
      } catch (e) {
        if (biPre) biPre.textContent = 'Error: ' + e.message;
      } finally {
  ...
  if (btnBiAudience && !btnBiAudience.dataset.bound) {
    btnBiAudience.addEventListener('click', async () => {
      ...
      try {
        ...
        try { await fetch('/api/analytics/event', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ type:'bi_ui_audience', data:{ days }})}); } catch {}
      } catch (e) {
        if (biPre) biPre.textContent = 'Error: ' + e.message;
      } finally {
  ...
  if (btnBiRoi && !btnBiRoi.dataset.bound) {
    btnBiRoi.addEventListener('click', async () => {
      ...
      try {
        ...
        try { await fetch('/api/analytics/event', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ type:'bi_ui_roi', data:{ days }})}); } catch {}
      } catch (e) {
        if (biPre) biPre.textContent = 'Error: ' + e.message;
      } finally {
...
(function(){
  ...
  async function refreshOutputList() {
    ...
    } catch (e) { console.warn('refreshOutputList error', e); }
  }
  ...
  if (keysnapBtn) {
    keysnapBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        alert('Error en KeySnap: ' + e.message);
      }
    });
  }
  ...
  if (ipfsBtn) {
    ipfsBtn.addEventListener('click', async () => {
      const f = await getSelectedFile();
      if (!f) { alert('Selecciona una pista para subir a IPFS.'); return; }
      try {
        ...
        alert('Error al subir a IPFS: ' + e.message);
      }
    });
  }
  ...
  if (fingerprintBtn) {
    fingerprintBtn.addEventListener('click', async () => {
      try {
        const f = await getSelectedFile();
        if (!f) { alert('Selecciona una pista (WAV) para fingerprint.'); return; }
        if (!/\.wav$/i.test(f)) { alert('Solo se soporta WAV para fingerprint.'); return; }
        ...
        try { await fetch('/api/analytics/event', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ type:'fingerprint_ui', file: f })}); } catch {}
        alert('Fingerprint creado. Abrir JSON: ' + url);
        try { window.open(url, '_blank', 'noopener'); } catch {}
      } catch (e) {
        alert('Error en Fingerprint: ' + e.message);
      }
    });
  }
  ...
  if (copyrightBtn) {
    copyrightBtn.addEventListener('click', async () => {
      try {
        ...
        if (!a || !b) { alert('Selecciona 2 pistas (WAV) para comparar.'); return; }
        if (!/\.wav$/i.test(a) || !/\.wav$/i.test(b)) { alert('Solo se soporta WAV para comparación.'); return; }
        ...
        try { await fetch('/api/analytics/event', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ type:'copyright_ui', data:{ fileA: a, fileB: b, sim: data.similarity, thr: data.threshold, potential: !!data.potentialCopy } })}); } catch {}
      } catch (e) {
        if (copyrightSpan) copyrightSpan.textContent = '';
        alert('Error en Check Copyright: ' + e.message);
      }
    });
  }
...
(function(){
  try {
    ...
    const actions = [
      ...
      {id:'searchGithub', label:'Buscar en GitHub…', run:()=>{document.getElementById('gh-query')?.focus()}},
      {id:'searchItunes', label:'Buscar en iTunes…', run:()=>{document.getElementById('itunes-query')?.focus()}},
      {id:'mixAI', label:'Componer con IA', run:()=>{document.getElementById('btn-composition')?.click()}},
      {id:'synthAI', label:'Generar Synth con IA', run:()=>{document.getElementById('btn-synth')?.click()}},
      {id:'drumsAI', label:'Generar Drums con IA', run:()=>{document.getElementById('btn-drums')?.click()}},
      {id:'keySnap', label:'Aplicar KeySnap', run:()=>{document.getElementById('btn-keysnap-apply')?.click()}},
      {id:'creditsSave', label:'Guardar créditos', run:()=>{document.getElementById('btn-cred-save')?.click()}},
      {id:'epkGenerate', label:'Generar EPK', run:()=>{document.getElementById('btn-epk-gen')?.click()}},
    ];
    ...
    function render(q=''){
      ...
      filtered.forEach((a, idx) => {
        ...
        if (idx === 0) { li.classList.add('active'); active = 0; }
    ...
    document.addEventListener('keydown', (e)=>{
      ...
      if (!dlg.classList.contains('hidden')){
        ...
        if (e.key === 'Enter'){
          e.preventDefault();
          if (active >= 0 && filtered[active]){ filtered[active].run(); close(); }
        }
      }
    });
  ...
  } catch (err) { console.warn('cmdk init error', err); }
})();
...
(function(){
  try {
    ...
    function getSum(){
      ...
      inputs.forEach(i => { const v = Number(i.value)||0; sum += v; count++; });
      return { sum, count };
    }
    function renderSum(){
...
(function(){
  try {
    ...
    function addSplitRow(party='', bps=''){
      ...
      const del = document.createElement('button'); del.type = 'button'; del.textContent = 'Eliminar';
      del.addEventListener('click', ()=>{ row.remove(); // actualizar suma
        splitsContainer.dispatchEvent(new Event('input', { bubbles: true }));
      });
      inBps.addEventListener('input', ()=>{ splitsContainer.dispatchEvent(new Event('input', { bubbles: true })); });
    ...
    function showResult(obj){ if (pre) pre.textContent = JSON.stringify(obj, null, 2); }
