...
const {
  generateComposition,
} = require("./src/generators/composition-generator");
...
async function callGeminiAPI(apiKey, prompt) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Error de Gemini: ${error.error?.message || JSON.stringify(error)}`,
    );
  }
...
async function identifySongWithACRCloud(audioBuffer) {
  ...
  if (result.status.code !== 0) {
    ...
    throw new Error(
      `Error de ACRCloud: ${result.status.msg} (código: ${result.status.code})`,
    );
  }
...
app.post(
  ...
  async (req, res) => {
    if (!req.file) {
      return res
        .status(400)
        .json({ message: "No se ha subido ningún archivo de audio." });
    }
...
app.post("/api/chatbot", async (req, res) => {
  ...
  if (!chatbotApiKey || !userMessage) {
    console.error(
      "Falta la clave API del chatbot en el servidor o el mensaje del usuario.",
    );
    return res
      .status(400)
      .json({
        message:
          "Falta el mensaje del usuario o hay un problema de configuración en el servidor.",
      });
  }
  ...
  try {
    ...
    if (!cleanResponse) {
      res.json({
        reply:
          "No he podido procesar esa pregunta. ¿Puedes intentar reformularla? 🤔",
      });
    }
...
app.post("/api/render-music", async (req, res) => {
  ...
  if (!musicData || !musicData.notes || !musicData.duration) {
    return res
      .status(400)
      .json({ message: "Faltan datos musicales (musicData) para renderizar." });
  }
...
app.post("/generate-creative", (req, res) => {
  // No necesita ser async ahora
  try {
    ...
    if (!params.key || !params.scale || !params.tempo) {
      return res
        .status(400)
        .json({ ok: false, message: "Faltan parámetros: key, scale o tempo." });
    }

    console.log(
      `Recibida petición para generar notas: ${params.key} ${params.scale} a ${params.tempo} BPM`,
    );
    ...
    res
      ...
      .json({
        ok: false,
        message: "Error interno del servidor al generar la música.",
      });
  }
});
...
app.post("/api/evolve-with-ai", async (req, res) => {
  try {
    ...
    if (!notes || !params) {
      return res
        .status(400)
        .json({ ok: false, message: "Faltan parámetros para evolucionar." });
    }
    const aiApiKey = process.env.AETHER_CHATBOT_API_KEY; // Reutilizamos la clave
    if (!aiApiKey) {
      throw new Error(
        "La clave API para la IA no está configurada en el servidor.",
      );
    }
    ...
    res
      ...
      .json({
        ok: false,
        message: "Error interno del servidor al evolucionar.",
      });
  }
});
...
app.post("/export-midi", (req, res) => {
  ...
  if (!notes || !tempo) {
    return res
      .status(400)
      .json({ ok: false, message: "Faltan datos de notas o tempo." });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ ok: false, message: "Error interno al crear el archivo MIDI." });
  }
});
