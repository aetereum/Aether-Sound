...
async function callN8N({ query, maxItems, overrides }) {
  ...
  if (String(DISABLE_N8N || '').toLowerCase() === 'true') {
    return { sources: [], note: 'n8n deshabilitado localmente (DISABLE_N8N=true)' };
  }
  const url = (overrides && overrides.n8nUrl) || N8N_WEBHOOK_URL;
  if (!url) {
    return { sources: [], note: 'N8N_WEBHOOK_URL no definido (ni override), se omite búsqueda externa' };
  }
  ...
  throw new Error(`n8n webhook error ${res.status}${errText ? `: ${errText}` : ''}`);
}
...
async function promptGemini(messages) {
  ...
  const body = {
    contents: [
      { role: 'user', parts: [{ text: messages.join('\n') }] }
    ]
  };
  const resGem = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
...
function buildPrompt({ query, sources }) {
  const srcText = sources?.map((s, i) => `${i + 1}. ${s.title || s.url || 'fuente'} — ${s.url || ''}`).join('\n') || 'Sin fuentes externas.';
  return [
...
function estimateDurationSec({ plan }) {
  ...
  const totalBars = (plan?.sections || []).reduce((acc, s) => acc + (s.bars || 0), 0) || 8;
...
async function renderFromPlan({ plan, outputDir }) {
  ...
  await composition.generateComposition({ outputDir, bpm, key, scale, durationSec });
...
async function searchAndCompose({ query, maxItems = parseInt(AI_MAX_ITEMS), outputDir, overrides }) {
  ...
  return { query, sources: sourcesPayload.sources || [], plan: renderInfo.plan, files: renderInfo.files };
}
...
async function assistModules({ module, context, selectedFiles = [] }) {
  ...
  if (!allowed.includes(mod)) {
    return { ok: false, error: 'Módulo inválido. Usa: creacion | publicacion | web3 | marketing' };
  }
